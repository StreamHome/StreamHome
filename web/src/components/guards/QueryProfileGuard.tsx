...
import { getProfiles, selectProfileAccess } from "../../api/profiles";
...
export function QueryProfileGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!requestedProfile || accessProfileId === requestedProfile.id || accessError) return;
    let cancelled = false;
    void selectProfileAccess(requestedProfile.id)
      .then(() => {
        if (cancelled) return;
        selectProfile(requestedProfile);
        setTheme(requestedProfile.theme);
        setAccessProfileId(requestedProfile.id);
      })
      .catch((requestError) => {
        if (!cancelled) setAccessError(requestError instanceof Error ? requestError.message : "Enter the profile PIN before continuing.");
      });
    return () => { cancelled = true; };
  }, [accessError, accessProfileId, requestedProfile, selectProfile, setTheme]);
  ...
  if (accessError) return <Navigate to="/profiles" state={{ from: location, error: accessError }} replace />;
  ...
  if (accessProfileId !== requestedProfile.id || activeProfile?.id !== requestedProfile.id) return <div className="app-guard-state" aria-label="Applying profile">Applying profile...</div>;
  return children;
}
