...
export function LoginPage() {
  ...
  return <motion.main className="login-page linear-ember-login" data-theme="ember" data-interaction="terminal" data-focus={focusedField} aria-busy={isLoading}>
    ...
    <motion.section className="login-auth-card" initial={reduced ? { opacity: 0 } : { opacity: 0, y: 22, scale: .985, filter: "blur(10px)" }} animate={{ opacity: 1, y: 0, scale: 1, filter: "blur(0px)" }} transition={{ duration: reduced ? MOTION_TIMINGS.reduced : .64, ease: MOTION_EASE }} aria-labelledby="login-title">
