...
function MediaCard({ movie, session, theme, onOpen }: { movie: Movie; session?: PlaybackSession; theme: string; onOpen: (movie: Movie) => void }) {
  ...
  return <motion.button layout="position" whileHover={{ y: -8, scale: 1.025 }} whileFocus={{ y: -5 }} transition={{ type: "spring", stiffness: 190, damping: 24 }} className="catalog-card" data-card-theme={theme} onClick={() => onOpen(movie)}><span className="catalog-card__art"><MediaArtwork src={movie.thumbnailUrl} alt={movie.title} media={movie} className="h-full w-full object-cover" /><i aria-hidden="true" /></span><span className="catalog-card__copy"><strong>{movie.title}</strong><small>{available ? `${movie.type} / ${movie.releaseYear || "catalogued"}` : "Unavailable on server"}</small></span>{session && <ProgressBar className="catalog-card__progress" progress={completionFraction(session.completionRate)} />}</motion.button>;
}
...
function MediaCollection({ title, label, items, sessions, theme, onOpen }: { title: string; label?: string; items: Movie[]; sessions: PlaybackSession[]; theme: string; onOpen: (movie: Movie) => void }) {
  ...
  return <section className="catalog-collection"><header><div>{label && <p>{label}</p>}<h2>{title}</h2></div><span>{String(items.length).padStart(2, "0")} titles</span></header><div className="catalog-collection__frame"><button className="catalog-rail-blade catalog-rail-blade--previous" onClick={() => scroll(-1)} aria-label={`Scroll ${title} backward`}>‹</button><div ref={rail} className="catalog-collection__rail">{items.map((movie) => <MediaCard key={movie.id} movie={movie} session={sessions.find((session) => session.movieId === movie.id)} theme={theme} onOpen={onOpen} />)}</div><button className="catalog-rail-blade catalog-rail-blade--next" onClick={() => scroll(1)} aria-label={`Scroll ${title} forward`}>›</button></div></section>;
}
...
function RotatingBillboard({ items, variant, context, onDetails, onPlay }: { items: Movie[]; variant: string; context: "home" | "movies" | "series"; onDetails: (movie: Movie) => void; onPlay: (movie: Movie) => void }) {
  ...
  return <div className="billboard-rotator" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setPaused(false); }}><AnimatePresence mode="wait" initial={false}><motion.div key={featured.id} initial={reduceMotion ? false : { opacity: 0, x: 22, filter: "blur(8px)" }} animate={{ opacity: 1, x: 0, filter: "blur(0px)" }} exit={reduceMotion ? undefined : { opacity: 0, x: -18, filter: "blur(6px)" }} transition={{ duration: reduceMotion ? 0 : 1.15, ease: [0.16, 1, 0.3, 1] }}><FeaturedStage movie={featured} variant={variant} context={context} onDetails={() => onDetails(featured)} onPlay={() => onPlay(featured)} /></motion.div></AnimatePresence>{rotationItems.length > 1 && <div className="billboard-pagination" aria-label="Featured media">{rotationItems.map((movie, itemIndex) => <button key={movie.id} data-active={itemIndex === index} onClick={() => setIndex(itemIndex)} aria-label={`Show ${movie.title}`} />)}</div>}</div>;
}
